...
function buildMvpSystemPrompt_(mvpCount, subMvpCount, totalMembers) {
  return '# タスク\n' +
    ...
    '## 制約（厳守）\n' +
    '入力データに書かれている事実のみを使うこと。以下を守れ。\n\n' +
    '### 使ってよい情報\n' +
    '- 得点数、勝利数、出場試合数 → 数字をそのまま使う\n' +
    '- チームメイトからのコメント → 内容を参考にし、自分の言葉で言い換える\n' +
    '- 備考欄の情報 → 参照OK、イジりのネタにしてもOK\n\n' +
    '### 禁止事項\n' +
    '- 入力データにない具体的プレー描写（例:「華麗なドリブル」「決定的なパス」）\n' +
    '- 推測表現（「〜に違いない」「きっと〜」「〜だったはず」「〜したことだろう」）\n' +
    '- チームメイトのコメントを「」でそのまま引用すること\n' +
    '- プロンプト内部の用語を出力に含めること（「備考」「備考欄」「評価方針」）\n\n' +
    '### 良い例・悪い例\n' +
    '× 「パスがうまい」というコメントが寄せられています\n' +
    '○ チームメイトからもパスセンスを絶賛されていた！\n\n' +
    '# 評価方針\n' +
    '- 社内フットサル。プロじゃない\n' +
    ...
    '## 採点基準（0.0〜10.0）\n' +
    '以下の評価軸を総合的に判断してスコアを付ける。得点や勝利だけが高評価の条件ではない。\n\n' +
    '### 評価軸（どれも同等に重要）\n' +
    '- チームメイトからの評価: コメントの数・ポジティブさが最も信頼できる指標\n' +
    '- 場への貢献: 盛り上げ、声かけ、助っ人参加など数字に表れない貢献\n' +
    '- 成長・チャレンジ: 未経験者や若手が積極的に参加していること自体が価値\n' +
    '- 試合結果: 得点数・勝利数は参考指標の一つ（これだけで高得点にしない）\n' +
    '- 参加姿勢: 出場試合数、助っ人での出場はプラス\n\n' +
    '### スコアの目安\n' +
    '- 9.0〜10.0: 複数の評価軸で突出。周囲からの評価も圧倒的\n' +
    '- 7.0〜8.9: 2つ以上の軸で目立つ貢献がある\n' +
    '- 5.0〜6.9: 参加して楽しんでいた。平均的な貢献\n' +
    ...
    '## 出力例（一般選手・rank空文字）\n' +
    '{"memberId": "m005", "rank": "", "title": "ムードメーカー", "reason": "", "rating": 5.8, "comment": "佐藤選手へ。出場3試合、チームの雰囲気を明るくしてくれたとチームメイトからの声が届いている⚽ 勝利こそ1つだったが、あなたがピッチにいるだけで周囲の士気が上がっていたのは間違いない事実だ。FIFA会長として、次回のさらなる飛躍を楽しみにしている💪"}\n\n' +
...
function buildMvpUserPrompt_(participantIds, mvpData) {
  ...
  return '以下の選手データに基づいて、全員分のMVP評価をJSON配列で出力してください。\n' +
    'システムプロンプトの出力仕様・制約を厳守すること。\n\n' +
    '# 入力データ\n' + memberLines.join('\n\n');
}
