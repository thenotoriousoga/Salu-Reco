...
- **Match を独立集約** に昇格(Round と分離)
- **Survey を Webフォーム自前化**(Google Forms 依存を削除)
- **SurveyResponse を独立集約** として分離
- コンテキスト境界の粒度は従来通り 6 つを維持
...
Read 側に限り、COUNT/EXISTS など集計用途のクロスコンテキスト参照を許容する(詳細は 05-backend-architecture.md)。
