...
- **AI分け**: サッカー経験（2pt）+ 年次スコア（最大1pt）で各メンバーをスコアリングし、蛇行ドラフト方式でNチームに均等分配
- **ランダム分け**: Fisher-Yatesシャッフル後、ラウンドロビンでNチームに分配
- 最低4人以上が必要
- チーム数は2〜最大（人数÷3）まで設定可能（1チーム最低3人）
- チーム名・スコアはJSON配列でラウンドシートに格納
- ラウンドメンバーのチーム列は `A`, `B`, `C`, `D`, `E` のアルファベットで管理
...
- 勝利判定はNチーム対応（単独トップスコアのチームに所属で勝利）
